...
export function generateMetadata(): Metadata {
  const title = "passfinal welcome"
  const description = "fast pass final exam"
  return {
    metadataBase: new URL(PROD_URL),
    title,
    description,
    applicationName: "passfinal",
  }
}
    ...
    <html lang="en">
      <body className={cn(inter.className, "antialiased bg-gray-100")}>
        ...
        >
          ...
          >
              <Image
                src="/_static/PassFinal-Logo.png"
                alt="App Store Badge"
                width={32}
                height={32}
                priority
          ...
          <div className="flex flex-row flex-nowrap gap-x-1.5 items-center">
            ...
            {/*</Link>*/}
            <Link
              href="https://www.techfens.com"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center"
            >
              <span className="sr-only">Github Repository</span>
              <ArrowUpRight size={20} />Blog
            </Link>
          </div>
        </header>
